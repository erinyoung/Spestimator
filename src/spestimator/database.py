...
# --- URLs ---
REFSEQ_16S_FASTA_URL = "https://ftp.ncbi.nlm.nih.gov/refseq/TargetedLoci/Bacteria/bacteria.16SrRNA.fna.gz"
...
def download_database(target_dir, force=False):
    ...
    try:
        ...
        if force or not fasta.exists():
            ...
            if fasta.exists(): fasta.unlink()
            
            success = download_file_with_progress(REFSEQ_16S_FASTA_URL, gz_fasta, desc="Downloading 16S FASTA")
        ...
        if gz_fasta.exists(): gz_fasta.unlink()
...
def fetch_refseq_assembly_summary():
    ...
    try:
        if not tmp_path.exists():
            success = download_file_with_progress(REFSEQ_ASSEMBLY_SUMMARY_URL, tmp_path, desc="Downloading Assembly Summary")
            if not success: return pd.DataFrame()
        else:
